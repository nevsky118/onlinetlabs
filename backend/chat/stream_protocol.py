...
def tool_input_start(tool_call_id: str, tool_name: str) -> str:
    ...
    return sse_event({"type": "tool-input-start", "toolCallId": tool_call_id, "toolName": tool_name})

def tool_input_delta(tool_call_id: str, delta: str) -> str:
    """SSE: дельта аргументов инструмента."""
    return sse_event({"type": "tool-input-delta", "toolCallId": tool_call_id, "inputTextDelta": delta})

def tool_input_available(tool_call_id: str, tool_name: str, input_data: dict) -> str:
    """SSE: полные аргументы инструмента готовы."""
    return sse_event({"type": "tool-input-available", "toolCallId": tool_call_id, "toolName": tool_name, "input": input_data})

def tool_output_available(tool_call_id: str, output: str) -> str:
    """SSE: результат выполнения инструмента."""
    return sse_event({"type": "tool-output-available", "toolCallId": tool_call_id, "output": output})
