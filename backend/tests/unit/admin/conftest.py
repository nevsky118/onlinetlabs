...
async def seeded_admin_db():
    ...
    async with factory() as db:
        db.add(User(id="adm-u1", email="adm@t.local", name="adm", role="student", control_arm="closed"))
        db.add(LearningSession(
            id="adm-sess-1", user_id="adm-u1", lab_slug="test-lab",
            status="ended",
        ))
        db.add(ExperimentMetrics(
            id="adm-em-1", session_id="adm-sess-1", user_id="adm-u1", lab_slug="test-lab",
            experiment_group="closed", base_arm="closed",
            total_time_seconds=1800.0, steps_completed=3, total_errors=1, repeated_errors=0,
            unique_error_types=1, interventions_received=1, interventions_succeeded=1,
            interventions_failed=0, interventions_accepted=1, escalations=0,
