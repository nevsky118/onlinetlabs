...
from consent.schemas import ConsentGrantRequest, ConsentResponse, ConsentRevokeResponse
from instructor.schemas import MCPAuditRow
from models.audit import Consent, MCPAudit
...
class TestConsentEndpointLogic:
    ...
    async def test_0838160b_grant_returns_consent(self, endpoint_db):
        ...
        with autotest.step("Assert: scope, observe, act set; revoked_at None"):
            resp = ConsentResponse.model_validate(consent)
    ...
    async def test_42b98f49_revoke_count(self, endpoint_db):
        ...
        with autotest.step("Act: revoke it"):
            revoked = await revoke(
                endpoint_db, "user-33333333-0000-0000-0000-000000000003", "study"
            )
        with autotest.step("Assert: 1 revoked, schema valid"):
            resp = ConsentRevokeResponse(revoked=revoked)
    ...
    async def test_732b1b92_audit_filter_kind(self, endpoint_db):
        ...
        with autotest.step("Assert: only act rows"):
            schemas = [MCPAuditRow.model_validate(row) for row in rows]
