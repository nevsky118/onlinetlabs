...
from kit.rate_limit import client_ip
...
from worker.reaper import reap_expired_sessions
...
class TestSessionReaper:
    ...
    async def setup_db(self):
        ...
        with patch("worker.reaper.async_session", self.session_factory):
            yield
    ...
    async def test_02fe1b05_expired_is_ended(self):
        ...
        with autotest.step("Act: run one reap pass"):
            with patch("worker.reaper.persist_volatile_configs", AsyncMock(return_value=1)):
    ...
    async def test_74a8d77a_live_session_survives(self):
        ...
        with autotest.step("Act: run one reap pass"):
            with patch("worker.reaper.persist_volatile_configs", AsyncMock(return_value=0)):
                ended = await reap_expired_sessions(gns3, registry)
...
class TestClientAddress:
    ...
    def test_c6fbb855_distinct_callers(self):
        ...
        with autotest.step("Act: resolve both"):
            first_key = client_ip(
                SimpleNamespace(headers={"x-forwarded-for": first.header}, client=None)
            )
            second_key = client_ip(
                SimpleNamespace(headers={"x-forwarded-for": second.header}, client=None)
            )

        with autotest.step("Assert: independent buckets"):
            assert_true(first_key != second_key, "different keys")
            assert_equal(first_key, first.expected, "first caller")
            assert_equal(second_key, second.expected, "second caller")
