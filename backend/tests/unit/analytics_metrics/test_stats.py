...
class TestPercentile:
    ...
    @autotest.name("percentile: p90 on 1..10 == 9.1 (Type 7, hand-computed)")
    def test_4080bdf9_p90_one_to_ten(self):
        ...
        with autotest.step("Act: percentile(data, 90)"):
            result = percentile([float(row) for row in data], 90)
    ...
    @autotest.name("percentile: p90 on 1..5 == 4.6 (Type 7, hand-computed)")
    ...
    @autotest.name("percentile: p50 on 1..10 == 5.5 (Type 7, hand-computed)")
    def test_75217610_p50_one_to_ten(self):
        ...
        with autotest.step("Act: percentile(data, 50)"):
            result = percentile([float(row) for row in data], 50)
    ...
    @autotest.name("percentile: a single point is returned as-is for any p")
    def test_4225c2f1_single_point(self):
        with autotest.step("Arrange: data=[7.0]"):
            data = [7.0]

        with autotest.step("Act: percentile for p50 and p90"):
            p50 = percentile(data, 50)
            p90 = percentile(data, 90)

        with autotest.step("Assert: both == 7.0 (no interpolation at n==1)"):
            assert_equal(p50, 7.0, "n==1 → p50 == the single value")
            assert_equal(p90, 7.0, "n==1 → p90 == the single value")

    @autotest.num("2534")
    @autotest.external_id("15da80aa-94e4-4c3d-9844-db18ab919bfc")
    @autotest.name("percentile: empty input → 0.0")
    def test_15da80aa_empty(self):
        with autotest.step("Act+Assert: percentile([], 90) == 0.0"):
            assert_equal(percentile([], 90), 0.0, "empty → 0.0")
