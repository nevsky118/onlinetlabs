...
from analytics.metrics.metrics import confusion_matrix
from analytics.metrics.scenarios import make_struggle_scenario
...
class TestConfusion:
    ...
    def test_9fc90dc3_no_detect(self):
        with autotest.step("Arrange: idle not detected"):
            scenario = make_struggle_scenario(ProcessRegime.IDLE, onset_index=4)
            pairs = [(scenario, Detection(False, None, None))]
        with autotest.step("Act"):
            cm = confusion_matrix(pairs)
