...
class TestNoGarbageFallback:
    ...
    async def test_c230cf00_hint_llm_failure_raises(self, config_model, monkeypatch):
    ...
    async def test_8eb78cde_hint_no_context_raises(self, config_model):
    ...
    async def test_efd15bff_tutor_llm_failure_raises(self, config_model, monkeypatch):
    ...
    async def test_3f542499_orchestrator_intervene_catches_agent_raise(
        self, config_model, monkeypatch
    ):
        ...
        with autotest.step("Assert: response is success=False with an error message"):
            assert_equal(resp.success, False, "success")
            assert_is_not_none(resp.error, "error")
