...
class TestSingleAgentAblation:
    ...
    @autotest.name("Single-agent mode: intervene forces generalist intervene_tutor")
    async def test_55d750d4_single_mode_routes_to_tutor(self, config_model):
        ...
        with autotest.step("Act: intervene (resolve_agent mocked → None, capture the route name)"):
            with patch(
                "agents.orchestrator.agent.resolve_agent", MagicMock(return_value=None)
            ) as ra:
                await orch.intervene(inp)

        with autotest.step("Assert: route is forced to intervene_tutor"):
    ...
    @autotest.name("Multi-agent (default): intervene routes by struggle type")
    async def test_6d88f4f3_multi_mode_routes_by_type(self, config_model):
        with autotest.step("Arrange: single_agent_mode=False (default), type=simplify"):
            orch = Orchestrator(config_model)
            inp = InterventionInput(
                session_id="s1",
                user_id="u1",
        ...
        with autotest.step("Assert: route by type intervene_simplify"):
            assert_equal(ra.call_args.args[0], "intervene_simplify", "multi → by type")
