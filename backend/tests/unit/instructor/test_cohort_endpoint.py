...
class TestCohortEndpoint:
    ...
    def test_af57dc57_cohort_response_from_result(self):
        ...
        with autotest.step("Assert: headline_arm and pooled.n"):
    ...
    def test_c32c4035_cohort_response_non_empty(self):
        ...
        with autotest.step("Act: aggregate and build the response"):
            out = aggregate_cohort([rec], horizon_seconds=7200.0, by_arm=False)
            resp = cohort_response_from_result(out)

        with autotest.step("Assert: by_skill contains a routing cell"):
            assert_equal(len(resp.by_skill), 1, "one cell in by_skill")
            cell = resp.by_skill[0]
            assert_equal(cell.skill, "routing", "skill=routing")
            assert_equal(cell.n, 1, "n=1")

        with autotest.step("Assert: reach_rate and reach_rate_at_horizon"):
        ...
        with autotest.step(
            "Assert: note survivorship-guard names the descriptive-trend disclaimer"
        ):
            assert_true("описатель" in cell.org_effect.note.lower(), "note contains 'описатель'")
