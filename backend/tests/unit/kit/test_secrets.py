...
from kit.secrets import _cipher, decrypt_secret, encrypt_secret
...
class TestSecrets:
    ...
    def test_390a784d_encrypt_decrypt_round_trip(self):
        with autotest.step("Encrypt a known plaintext"):
            plaintext = "super-secret-password-123"
            token = encrypt_secret(plaintext)

        with autotest.step("Decrypt it back"):
            assert_equal(decrypt_secret(token), plaintext, "round-trip matches")

    @autotest.num("741")
    @autotest.external_id("01ebaa5c-008a-4abf-acf5-1e5802fceb72")
    @autotest.name("Secrets: corrupted token -> InvalidToken")
    def test_01ebaa5c_corrupted_token_raises(self):
        with autotest.step("Encrypt and corrupt the middle of the token"):
            token = encrypt_secret("plain")
            corrupted = token[:5] + "XXXXX" + token[10:]

        with autotest.step("Decrypting the corrupted block raises InvalidToken"):
            with pytest.raises(InvalidToken):
                decrypt_secret(corrupted)

    @autotest.num("742")
    @autotest.external_id("8d4d5f16-a427-4b9f-82eb-2e6d7e36a12c")
    @autotest.name("Secrets: decrypting with the wrong key -> InvalidToken")
    def test_8d4d5f16_wrong_key_raises(self):
        with autotest.step("Encrypt under one key"):
            other_key = Fernet.generate_key().decode()
            other_cipher = Fernet(other_key.encode())
            token = other_cipher.encrypt(b"plain").decode()

        with autotest.step("Try to decrypt with the app's default key"):
            with pytest.raises(InvalidToken):
