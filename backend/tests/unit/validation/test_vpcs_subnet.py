...
class TestVpcsSubnet:
    ...
    def test_beafd3af_parse_show_ip_reads_ip_and_gateway(self):
        ...
        with autotest.step("Assert: ip and gateway are extracted"):
            assert_equal(parsed["ip"], "192.168.10.10/24", "ip")
            assert_equal(parsed["gateway"], "192.168.10.1", "gateway")
    ...
    def test_370bc14b_parse_show_ip_garbage(self):
        ...
        with autotest.step("Assert: empty ip and gateway"):
            assert_equal(parsed, {"ip": "", "gateway": ""}, "parsed")
    ...
    def test_008caad4_ip_in_subnet(self, ip_with_mask, subnet, ok):
        with autotest.step("Act+Assert: _ip_in_subnet matches the expected verdict"):
            assert_true(
                _ip_in_subnet(ip_with_mask, subnet) is ok,
                "_ip_in_subnet(ip_with_mask, subnet) is ok",
            )
