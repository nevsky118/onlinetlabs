...
class TestLatencyCapture:
    ...
    async def test_6356775e_record_and_aggregate(self):
        with autotest.step("Arrange: record 10 analysis-stage durations"):
            from analytics.runtime.latency import record_stage_latency, stage_percentiles
    ...
    async def test_aa995048_stages_isolated(self):
        ...
        with autotest.step("Act+Assert: each stage is counted separately"):
            async with sf() as db:
                percentiles = await stage_percentiles(db, "analysis", [50])
                percentiles_2 = await stage_percentiles(db, "intervention", [50])
            assert_equal(percentiles[50], 100.0, "analysis p50 == 100")
            assert_equal(percentiles_2[50], 10.0, "intervention p50 == 10")
