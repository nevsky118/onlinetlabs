...
from mcp_sdk.testing.custom_assertions import assert_equal, assert_greater_equal
...
def _e(action, cid, actual, value):
...
class TestFeaturesProgress:
    @autotest.num("3227")
    @autotest.external_id("2e397602-fa23-444a-aaf3-45394fca19f4")
    @autotest.name(
        "FeatureExtractor.compute: counts distinct failing actuals and the unchanged tail"
    )
    def test_2e397602_distinct_actuals_and_unchanged_run(self):
        with autotest.step(
            "Arrange: two retries with different actuals, then two unchanged failures"
        ):
        ...
        with autotest.step("Act: compute"):
            features = FeatureExtractor().compute("s1", evs)

        with autotest.step("Assert: distinct actuals counted, unchanged tail counted"):
            assert_greater_equal(features.distinct_failing_actuals, 2, "distinct failing actuals")
            assert_equal(features.cycles_failing_unchanged, 2, "cycles failing unchanged")
    ...
    def test_7ae0b190_empty_events_returns_zero(self):
        with autotest.step("Act: compute over an empty event list"):
            features = FeatureExtractor().compute("s1", [])

        with autotest.step("Assert: both counters are zero"):
            assert_equal(features.distinct_failing_actuals, 0, "distinct failing actuals")
            assert_equal(features.cycles_failing_unchanged, 0, "cycles failing unchanged")
    ...
    def test_c6e216e7_no_check_actions_returns_zero(self):
    ...
    @autotest.name(
        "FeatureExtractor.compute: cycles_failing_unchanged tail breaks on component change"
    )
    def test_c586543a_cycles_broken_by_different_component(self):
    ...
    def test_b17f600a_cycles_broken_by_check_passed(self):
    ...
    def test_4858c12b_error_run_reset_by_check_passed(self):
        ...
        with autotest.step("Assert: error_repeat_count resets to 0"):
            assert_equal(features.error_repeat_count, 0, "error repeat count")
    ...
    def test_3cc0cc83_error_run_accumulates_without_check_passed(self):
        ...
        with autotest.step("Assert: error_repeat_count accumulates"):
            assert_greater_equal(features.error_repeat_count, 2, "error repeat count")
