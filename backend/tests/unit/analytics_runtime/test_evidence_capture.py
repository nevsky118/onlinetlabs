...
def _collector(sf, *, evidence_enabled):
    ...
    collector._session_id = "s1"
    collector._user_id = "u1"
    collector._lab_slug = "lab-gns3"
    collector._ctx = MagicMock()
    return collector
...
class TestEvidenceCapture:
    ...
    async def test_87aa2cb9_capture_snapshot_json_safe(self):
        with autotest.step("Arrange: real sqlite"):
            from analytics.runtime.evidence import capture_snapshot
    ...
    async def test_428437ae_poll_captures_when_enabled(self):
        with autotest.step("Arrange: collector with capture, _persist mocked, events present"):
            sf = await _sqlite_factory()
            collector = _collector(sf, evidence_enabled=True)
            collector._persist = AsyncMock()

        with autotest.step("Act: _poll_cycle with one action"):
            with (
                patch.object(
                    collector, "_fetch_actions", AsyncMock(return_value=[{"action": "ping"}])
                ),
                patch.object(collector, "_fetch_logs", AsyncMock(return_value=[])),
                patch.object(collector, "_fetch_errors", AsyncMock(return_value=[])),
            ):
    ...
    async def test_ea2133f1_poll_no_capture_when_disabled(self):
        with autotest.step("Arrange: collector without capture"):
            sf = await _sqlite_factory()
            collector = _collector(sf, evidence_enabled=False)
            collector._persist = AsyncMock()
