...
from analytics.runtime.process_state import ProcessRegime, analysis_to_regime, is_bad
...
class TestProcessState:
    ...
    def test_e1f4efe4_productive_when_no_struggle(self):
        with autotest.step("Arrange: analysis with no struggle"):
            analysis = SimpleNamespace(struggle_detected=False, struggle_type=None)
    ...
    def test_a08292ae_regime_mirrors_struggle_type(self):
        with autotest.step("Arrange: analysis with STUCK_ON_STEP struggle"):
            analysis = SimpleNamespace(
                struggle_detected=True, struggle_type=StruggleType.STUCK_ON_STEP
            )
        ...
        with autotest.step("Assert: regime STUCK_ON_STEP, value=stuck_on_step, is_bad=True"):
            assert_equal(regime, ProcessRegime.STUCK_ON_STEP, "regime STUCK_ON_STEP")
            assert_equal(regime.value, "stuck_on_step", "enum value stuck_on_step")
            assert_true(is_bad(regime), "STUCK_ON_STEP is a bad regime")
