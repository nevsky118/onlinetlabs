...
from mcp_sdk.testing.custom_assertions import assert_in, assert_is_none, assert_true
...
from observability.schemas import ActivityKind
...
class TestMonitorEmits:
    ...
    async def test_75ce6cbf_emits_struggle_detected(self, config_model):
        ...
        with autotest.step("Check that emit was called with STRUGGLE_DETECTED"):
            assert_true(activity.emit.called, "emit was not called")
            emitted_kinds = [call.args[0].kind for call in activity.emit.call_args_list]
            assert_in(ActivityKind.STRUGGLE_DETECTED, emitted_kinds, "STRUGGLE DETECTED")
    ...
    async def test_724254ea_emits_cooldown_skip(self, config_model):
        ...
        with autotest.step("Check STRUGGLE_DETECTED and COOLDOWN_SKIP"):
            assert_is_none(result, "the cooldown blocks the intervention")
            emitted_kinds = [call.args[0].kind for call in activity.emit.call_args_list]
            assert_in(ActivityKind.STRUGGLE_DETECTED, emitted_kinds, "STRUGGLE_DETECTED is missing")
            assert_in(ActivityKind.COOLDOWN_SKIP, emitted_kinds, "COOLDOWN_SKIP is missing")
