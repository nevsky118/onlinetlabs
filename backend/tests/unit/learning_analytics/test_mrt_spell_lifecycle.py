"""MRT: жизненный цикл spell на реальной sqlite — exit_ts при закрытии + censoring."""
...
async def _sqlite_factory():
    # Создаём только нужные таблицы: Base.metadata.create_all падает на JSONB
    # (platform_events) под sqlite. Монитор пишет ProcessStateSample (_log_process_state)
    # и BehavioralEvent (_log_would_intervene).
...
def _analysis(*, struggle):
    ...
    return AnalyticsResult(
        ...
        confidence=0.9, suggested_intervention=SuggestedIntervention.HINT,
        difficulty_recommendation=DifficultyRecommendation(
            ...
            metrics=StudentMetrics(total_attempts=5, success_rate=0.6,
...
def _monitor(session_factory):
    ...
    cfg.mrt_hold_probability = 1.0  # всегда withhold — не тянем dispatch в этом тесте
    cfg.mrt_t_k_jitter_frac = 0.0
    m = SessionMonitor(
        mcp_client=MagicMock(), db_factory=session_factory, orchestrator=MagicMock(),
...
class TestMRTSpellLifecycle:
    ...
    @autotest.name("MRT: смена режима на productive закрывает spell — проставляет subsequent_exit_ts")
    async def test_472db8c1_spell_exit_sets_exit_ts(self):
        ...
        with autotest.step("Act: цикл 1 (idle → открыть spell + записать точку), цикл 2 (productive → закрыть)"):
            with (
        ...
        with autotest.step("Assert: точка решения получила subsequent_exit_ts"):
            async with sf() as db:
                rows = (await db.execute(
                    select(InterventionDecision).where(InterventionDecision.session_id == "s1")
                )).scalars().all()
            assert_equal(len(rows), 1, f"ровно 1 точка решения; получено {len(rows)}")
            assert_true(rows[0].subsequent_exit_ts is not None,
        ...
        with autotest.step("Arrange: одна открытая точка (exit_ts null) и одна закрытая"):
            ...
            async with sf() as db:
                db.add(InterventionDecision(
                    id="d-open", session_id="s1", user_id="u1", lab_slug="lab",
                    spell_id="sp1", ts=now, regime="idle", dwell_seconds=10.0,
                    t_k_applied=0.0, assignment="withhold", subsequent_exit_ts=None,
                ))
                db.add(InterventionDecision(
                    id="d-closed", session_id="s1", user_id="u1", lab_slug="lab",
                    spell_id="sp0", ts=now, regime="idle", dwell_seconds=5.0,
                    t_k_applied=0.0, assignment="intervene", subsequent_exit_ts=now,
                ))
                await db.commit()
