...
class TestSessionEndpoints:
    ...
    async def test_f4ab80d0_list_returns_sessions_and_count(self):
        ...
        with autotest.step("Assert: 200, count=2, only user-a's sessions"):
    ...
    async def test_696f19c2_delete_single_session(self):
        ...
        with autotest.step("Assert: GET returns only s2"):
            async with self._client() as client:
                list_resp = await client.get("/users/me/auth-sessions")
            assert_equal(list_resp.json()["count"], 1, "1 session left")
    ...
    async def test_94daf388_delete_foreign_or_missing_404(self):
        ...
        with autotest.step("Assert: 404 for someone else's session"):
            assert_equal(resp_foreign.status_code, 404, "foreign → 404")

        with autotest.step("Act: user-a deletes a nonexistent session"):
            async with self._client() as client:
                resp_missing = await client.delete("/users/me/auth-sessions/does-not-exist")

        with autotest.step("Assert: 404 for a nonexistent session"):
            assert_equal(resp_missing.status_code, 404, "nonexistent → 404")
    ...
    async def test_e838e96a_delete_all_sessions(self):
        ...
        with autotest.step("Assert: GET returns count=0"):
            async with self._client() as client:
                list_resp = await client.get("/users/me/auth-sessions")
            assert_equal(list_resp.json()["count"], 0, "count=0 after deleting all")
