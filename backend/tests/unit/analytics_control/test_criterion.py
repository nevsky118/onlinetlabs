...
from analytics.criterion import Costs, compute_J
...
class TestCriterion:
    ...
    @autotest.name("compute_J: correctly sums bad_duration and interventions")
    def test_85394eae_J_sums_duration_and_interventions(self):
        with autotest.step("Arrange: two stuck_on_step samples, one intervention"):
            samples = [
        ...
        with autotest.step("Assert: bad_duration=60, n_interventions=1, n_false=0, J=62"):
            ...
            assert_equal(res.n_false, 0, "n_false == 0 (no clean exits)")
            assert_equal(res.J, 60.0 * 1.0 + 1 * 2.0 + 0 * 0.5, "J == 62.0")

    @autotest.num("1423")
    @autotest.external_id("0c443323-2e38-477c-9aa4-53c2a5c15713")
    @autotest.name("compute_J: with no interventions J=0, n_false=0")
    def test_0c443323_no_interventions_zero_false(self):
        with autotest.step("Arrange: one productive sample, empty interventions"):
    ...
    def test_00208a1c_count_false_flags_premature_intervention(self):
        with autotest.step("Arrange: three 60s clean spells, one with an intervention at 20s"):
            ...
            def _spell(start: int, duration: int, with_iv: bool) -> list[dict]:
                ...
                result = []
                start_2 = start
                while start_2 < start + duration:
                    result.append(
                        {"ts": start_2, "regime": "stuck_on_step", "dwell": float(start_2 - start)}
                    )
                    start_2 += 10
                result.append({"ts": start_2, "regime": "productive", "dwell": 0.0})
                return result
        ...
        with autotest.step("Assert: n_false==1, J > bad_duration + n_interventions*c_int"):
            assert_equal(res.n_false, 1, f"expected 1 false, got {res.n_false}")
            assert_true(
                res.bad_duration + res.n_interventions * costs.c_intervention < res.J,
                "false intervention penalty is included in J",
            )
