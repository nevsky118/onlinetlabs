...
class TestSessionInterventionsWsOwnership:
    ...
    @autotest.name("session_interventions_ws: чужая сессия — close(4404), gateway.connect не вызывается")
    async def test_c205bb8c_rejects_session_not_owned_by_user(self):
        with autotest.step("Arrange: валидный токен, get_session возвращает None (сессия не принадлежит user-1)"):
            gateway = _fake_gateway()
            fake_ws = _FakeWebSocket(gateway)

        with autotest.step("Act: подключение к чужому session_id"):
            with patch("sessions.routers.ws.decode_backend_token", return_value={"sub": "user-1"}), \
    ...
    @autotest.name("session_interventions_ws: владелец сессии — gateway.connect вызывается, disconnect по обрыву")
    async def test_9a0f526e_connects_owner_to_gateway(self):
        ...
        with autotest.step("Assert: gateway.connect вызван с (session_id, websocket), disconnect — при обрыве связи"):
