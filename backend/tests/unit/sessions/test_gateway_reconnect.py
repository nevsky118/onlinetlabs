...
class TestGatewayReconnect:
    ...
    @autotest.name("disconnect: отложенный disconnect старого сокета не вытесняет новый после reconnect")
    async def test_501b5e1f_stale_disconnect_does_not_evict_new_socket(self):
        ...
        with autotest.step("Act: connect(старый) → connect(новый, reconnect) → disconnect(старый) с опозданием"):
        ...
        with autotest.step("Assert: новый сокет остался в _connections"):
            assert_true(gw._connections.get("s1") is ws_new, "новый сокет не вытеснен старым disconnect")
    ...
    @autotest.name("disconnect: без websocket-аргумента сохраняет старое поведение (удаляет без проверки)")
