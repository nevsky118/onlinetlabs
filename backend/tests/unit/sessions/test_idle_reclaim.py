...
from sessions.idle_reclaim import IDLE_THRESHOLD_MIN, _reclaim_idle_sessions
...
class TestReclaimIdleSessions:
    ...
    async def setup_db(self):
        ...
        async with self.engine.begin() as conn:
            await conn.run_sync(User.__table__.create)
            await conn.run_sync(LearningSession.__table__.create)
        with patch("sessions.idle_reclaim.async_session", self.session_factory):
            yield
        await self.engine.dispose()
    ...
    async def test_e49fb50e_recent_activity_is_spared(self):
        ...
        with autotest.step("Act: run one reclaim pass"):
            with patch("sessions.idle_reclaim.persist_volatile_configs", AsyncMock(return_value=2)):
    ...
    async def test_c51bcc0b_saves_before_stop(self):
        ...
        with autotest.step("Act: run one reclaim pass, recording the call order"):
            ...
            with patch("sessions.idle_reclaim.persist_volatile_configs", _persist):
                await _reclaim_idle_sessions(client)
    ...
    async def test_c67914da_paused_is_skipped(self):
        ...
        with autotest.step("Act: run one reclaim pass"):
            with patch("sessions.idle_reclaim.persist_volatile_configs", AsyncMock(return_value=0)):
                await _reclaim_idle_sessions(client)
