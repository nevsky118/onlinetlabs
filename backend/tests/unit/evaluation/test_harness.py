...
class TestHarness:
    ...
    def test_a3f7c21e_detects_struggle(self):
        ...
        with autotest.step("Assert: detected, ts>=onset, type repeating"):
            assert_true(d.detected, "detected")
            assert_greater_equal(d.detected_ts, 90.0, "not before onset")
            assert_equal(d.detected_regime, ProcessRegime.REPEATING_ERRORS, "type")
    ...
    def test_b4e8d32f_normal_no_detect(self):
        ...
        with autotest.step("Assert: no detection"):
            assert_false(d.detected, "no false positive")
