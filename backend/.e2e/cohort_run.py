"""Cohort-metrics demo against a real database.

Progress, sessions and metrics are seeded; compute_cohort_metrics is production
code. Exercises the KM fallback, censoring and strata.
"""
...
async def run():
    async with async_session() as db:
        # 9 closed students: static-ip (4 reached L2, 2 censored) + dhcp (3 censored, no L2 pair)
        ...
        em_specs = []  # (uid, slug, l2_pass, session_id); metrics inserted after sessions commit (FK)
        # static-ip: reached L2 on days 5, 7, 9, 11
    ...
    def _d(sec):
        return "-" if sec is None else f"{sec / DAY:.1f}d"

    print(f"\n=== Cohort metrics (headline={out['headline_arm']}) ===")
    print(
        "| Stratum | n | reach L2 | censored | median calendar | reach@30d | interventions L1->L2 |"
    )
    print("|-|-|-|-|-|-|-|")
    for cell in out["by_skill"] + [out["pooled"]]:
        t, a = cell.time_to_competence, cell.autonomy
        label = cell.skill or "POOLED"
        l2i = "—" if a.mean_l2_interventions is None else f"{a.mean_l2_interventions:.1f}"
        print(
            f"| {label} | {t.n} | {t.reach_rate:.2f} | {t.censored} | {_d(t.median_calendar_seconds)} "
            f"| {t.reach_rate_at_horizon:.2f} | {a.mean_l1_interventions:.1f}→{l2i} |"
        )
    print(f"\narms by stratum: {[c.arm for c in (out['by_arm'] or [])]}")
...
async def cleanup():
    async with async_session() as db:
        ...
        await db.commit()
    print("=== demo data removed (CASCADE) ===")
