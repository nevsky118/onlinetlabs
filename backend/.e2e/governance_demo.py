"""ControlInterface governance demo against a real database.

MCP is stubbed: this exercises the gates, not GNS3. Consent, isolation and audit
hit the real database. Prints which calls pass, which are denied and why.
"""
...
class _FakeMCP:
    """Stub MCP: the gates are real, the call itself is not."""
...
async def run():
    ...
    u1, s1 = f"{PREFIX}u1", f"{PREFIX}s1"  # has consent, owns s1
    u2, s2 = f"{PREFIX}u2", f"{PREFIX}s2"  # no consent, owns s2
    ...
    async with async_session() as db:
        ...
        await db.commit()  # users before sessions (FK)
        db.add(
            LearningSession(
                id=s1,
                user_id=u1,
        ...
        await db.commit()
        await grant(db, u1, "study", observe=True, act=True)  # study scope covers observe + act
    ...
    await _try(
        "observe list_user_actions (u1, своя сессия, согласие)",
        ci.observe(
            "list_user_actions", {}, {}, user_id=u1, session_id=s1, lab_slug="lan-static-ip"
        ),
    )
    await _try(
        "act execute_action (u1, closed-плечо, согласие)",
        ci.act(
            "execute_action",
            {},
            {"action_name": "ping", "params": {}},
            user_id=u1,
            session_id=s1,
    ...
    await _try(
        "act execute_action (u1, OPEN-плечо — проактив подавлен)",
        ci.act(
            "execute_action",
            {},
            {"action_name": "ping"},
    ...
    await _try(
        "act execute_action (u1, повтор сразу — rate-backstop = cooldown)",
        ci.act(
            "execute_action",
            {},
            {"action_name": "ping"},
    ...
    await _try(
        "observe (u2, своя сессия, НО без согласия)",
        ci.observe("list_user_actions", {}, {}, user_id=u2, session_id=s2),
    )
    await _try(
        "observe (u1 пытается читать ЧУЖУЮ сессию s2 — изоляция)",
        ci.observe("list_user_actions", {}, {}, user_id=u1, session_id=s2),
    )
    await _try(
        "observe неклассифицированный инструмент rm_rf_all (default-deny)",
    ...
    async with async_session() as db:
        rows = (
            (
                await db.execute(
                    select(MCPAudit)
                    .where(MCPAudit.user_id.like(f"{PREFIX}%"))
                    .order_by(MCPAudit.ts)
                )
            )
            .scalars()
            .all()
