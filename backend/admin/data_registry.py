...
ADMIN_TABLES: dict[str, TableSpec] = {
    "mcp_audit": TableSpec(
        model=MCPAudit,
        columns=["id", "user_id", "session_id", "tool", "kind", "ts", "success", "error", "consent_ref", "lab_slug"],
    ...
    "agent_activity_events": TableSpec(
        ...
        columns=["id", "session_id", "user_id", "ts", "source", "kind", "agent", "severity", "summary"],
        sortable={"id", "ts", "session_id", "user_id", "severity"},
        searchable=["id", "session_id", "user_id", "source", "kind", "agent", "severity", "summary"],
        masked=set(),
        default_sort="ts",
    ),
    "platform_events": TableSpec(
        model=PlatformEvent,
        columns=["id", "event_name", "user_id", "session_id", "device_id", "properties", "client_ts", "server_ts"],
    ...
    "behavioral_events": TableSpec(
        ...
        columns=["id", "session_id", "user_id", "lab_slug", "timestamp", "event_type", "component_id", "component_type", "action", "raw_command", "success", "severity", "message"],
        sortable={"id", "timestamp", "session_id", "user_id", "event_type"},
        searchable=["id", "session_id", "user_id", "lab_slug", "event_type", "component_id", "component_type", "action", "raw_command", "severity", "message"],
        masked=set(),
    ...
    "consents": TableSpec(
        model=Consent,
        columns=["id", "user_id", "scope", "observe", "act", "granted_at", "revoked_at", "data_policy"],
    ...
    "lab_progress": TableSpec(
        model=LabProgress,
        columns=["id", "user_id", "lab_slug", "status", "score", "current_step", "started_at", "completed_at", "updated_at"],
    ...
    "course_progress": TableSpec(
        ...
        columns=["id", "user_id", "course_slug", "status", "score", "started_at", "completed_at", "updated_at"],
        sortable={"id", "updated_at", "user_id", "status"},
    ...
    "step_attempts": TableSpec(
        model=StepAttempt,
        columns=["id", "user_id", "lab_slug", "step_slug", "attempt_number", "result", "score", "started_at", "ended_at"],
    ...
    "process_state_samples": TableSpec(
        ...
        columns=["id", "session_id", "user_id", "lab_slug", "ts", "regime", "dwell_seconds", "created_at"],
    ...
    "experiment_metrics": TableSpec(
        ...
        columns=[
            "id", "session_id", "user_id", "lab_slug", "experiment_group", "agent_backend",
            "total_time_seconds", "steps_completed", "total_errors", "repeated_errors",
            "unique_error_types", "interventions_received", "interventions_succeeded",
            "interventions_failed", "interventions_accepted", "control_arm", "base_arm",
            "escalations", "would_interventions", "l1_interventions", "l2_unassisted_pass",
            "final_score", "completed", "completed_at", "created_at",
        ],
        sortable={"id", "created_at", "session_id", "user_id", "final_score"},
        searchable=["id", "session_id", "user_id", "lab_slug", "experiment_group", "agent_backend", "control_arm", "base_arm"],
        masked=set(),
        default_sort="created_at",
    ),
}
