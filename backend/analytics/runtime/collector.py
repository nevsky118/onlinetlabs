"""BehavioralCollector polls the MCP server and stores behavioral events."""
...
class BehavioralCollector:
    ...
    async def _fetch_errors(self) -> list[dict]:
        ...
        result: list[dict] = []
        try:
            errors = await self._call_observe("list_errors", {"since": self._last_error_poll})
            self._last_error_poll = datetime.now(tz=UTC)
            for err in errors:
                result.append(
                    self.normalize_error_entry(
                        err,
                        self._session_id,
                        self._user_id,
                        self._lab_slug,
