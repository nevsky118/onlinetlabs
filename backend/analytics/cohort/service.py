...
async def _index_sessions_by_skill(sessions: list, skill_cache: dict, db) -> dict:
    """(user_id, skill) -> that learner's sessions, built in one pass.

    Previously every (learner, skill) pair rescanned the whole session list, which
    is quadratic in the cohort size.
    """
    index: dict = {}
    for s in sessions:
        skill = await _skill_of(db, s.lab_slug, skill_cache)
        if skill is None:
            continue
        index.setdefault((s.user_id, skill), []).append(s)
    return index
...
async def compute_cohort_metrics(db, horizon_seconds: float, by_arm: bool = False) -> dict:
    ...
    sessions_by_skill = await _index_sessions_by_skill(sessions, skill_cache, db)
    ...
    for (user_id, skill), labs in grouped.items():
        ...
        user_skill_sessions = sessions_by_skill.get((user_id, skill), [])
