...
from simulation.ground_truth import SIM_TRUTH_CODER
...
async def inter_rater_kappa(db: AsyncSession, session_id: str, coder_a: str, coder_b: str) -> float:
...
async def gold_label_count(db: AsyncSession, session_id: str | None = None) -> int:
    """Count of human adjudicated-gold annotations (optionally scoped to one session).

    Simulated ground truth shares the table and the is_gold flag; it is not a
    human label and must not inflate the count.
    """
    stmt = (
        select(func.count())
        ...
        .where(
            ...
            RegimeAnnotation.coder_id != SIM_TRUTH_CODER,
        )
    )
