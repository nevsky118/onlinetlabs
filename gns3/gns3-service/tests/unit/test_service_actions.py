...
from tests.settings.data.gns3_data import Gns3ProjectData, Gns3UserData
...
class TestSessionServiceNodeAction:
    ...
    async def test_6c7c2763_node_action_calls_admin_and_invalidates_cache(self, service, admin):
        ...
        with autotest.step("Assert: admin was called and the cached state was invalidated"):
            ...
            assert_is_none(
                service._state_cache.get("11111111-1111-1111-1111-111111111111"),
                "cache invalidated",
            )
...
class TestSessionServiceCreateSession:
    ...
    async def test_3f395441_create_session_happy_path(self):
        with autotest.step("Arrange: admin stubs for a fresh user, project, role and token"):
            admin = AsyncMock()
            admin.find_user_by_name.return_value = None
            user = Gns3UserData(user_id="u-42", username="student-abcdef12").data
            project = Gns3ProjectData(project_id="proj-99").data
        ...
        with autotest.step("Assert: user, project, ACLs and session response are all correct"):
            ...
            assert_equal(username_arg, expected_username, "username arg")
            ...
            acl_calls = [call.args for call in admin.create_acl.await_args_list]
            assert_equal(
                acl_calls,
                [
            ...
            assert_equal(response.project_id, "proj-99", "project id")
            assert_equal(response.gns3_user_id, "u-42", "gns3 user id")
            assert_equal(response.gns3_username, expected_username, "gns3 username")
            assert_equal(response.gns3_jwt, "jwt-token-xyz", "gns3 jwt")
            assert_equal(response.gns3_url, "http://gns3:3080", "gns3 url")
            assert_in("proj-99", response.gns3_deep_url, "'proj-99'")
    ...
    async def test_7f17d6c0_create_session_propagates_admin_error_and_rolls_back(
        ...
        with autotest.step("Arrange: user and project succeed, opening the project fails"):
            admin = AsyncMock()
            admin.find_user_by_name.return_value = None
            admin.create_user.return_value = Gns3UserData(user_id="u-42").data
            admin.duplicate_project.return_value = Gns3ProjectData(project_id="p-99").data
...
class TestSessionServiceDeleteSession:
    ...
    async def test_46892655_delete_session_finally_marks_closed_even_on_admin_error(self):
        ...
        with autotest.step("Assert: the session is marked closed and committed regardless"):
            assert_equal(session.status, SessionStatus.CLOSED, "status")
            assert_is_not_none(session.closed_at, "closed at")
            db.commit.assert_awaited_once()
...
class TestSessionServiceDeleteStopsNodes:
    ...
    async def test_57a08a06_delete_session_stops_nodes_first(self):
        ...
        with autotest.step("Assert: nodes were stopped, and before the project was deleted"):
            ...
            assert_equal(order, ["stop", "delete_project"], "order")
