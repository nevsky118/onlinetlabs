...
from mcp_sdk.testing.custom_assertions import assert_equal, assert_not_equal
...
class TestRouterAuth:
    ...
    def test_c18bfbf6_guarded_paths_reject_missing_token(self, client, path):
        with autotest.step("Act+Assert: request without the internal token is rejected with 403"):
            assert_equal(client.get(path).status_code, 403, "status code")
    ...
    def test_a332ad01_guarded_paths_reject_wrong_token(self, client, path):
    ...
    def test_5fe99ff7_guarded_paths_reject_malformed_authorization(self, client):
    ...
    def test_0edff9c7_health_stays_open(self, client):
        ...
        with autotest.step("Act+Assert: /health without a token is not rejected with 403"):
            assert_not_equal(client.get("/health").status_code, 403, "status code")
