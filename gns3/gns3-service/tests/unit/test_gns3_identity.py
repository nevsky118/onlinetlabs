...
from mcp_sdk.testing.custom_assertions import assert_equal, assert_not_equal, assert_true
...
class TestGns3UsernameFor:
    ...
    def test_1df13869_distinct_users_with_shared_prefix_get_distinct_names(self):
        ...
        with autotest.step("Act: build a name for the first id"):
            first = gns3_username_for("sim-12000-1")

        with autotest.step("Act: build a name for a second id sharing the same prefix"):
            second = gns3_username_for("sim-12000-2")

        with autotest.step("Act: build a name for a third id sharing the same prefix"):
            third = gns3_username_for("sim-12000-49")

        with autotest.step("Assert: all three names are distinct"):
            assert_true(first != second != third, "three distinct names")
            assert_equal(
                len({first, second, third}), 3, "names must differ despite a shared prefix"
            )
    ...
    def test_08d3d5a7_is_deterministic(self):
        ...
        with autotest.step("Act + Assert: the same id yields the same name twice"):
            assert_equal(
                gns3_username_for("user-abc"), gns3_username_for("user-abc"), "gns3 username for"
            )
    ...
    def test_785aaaf0_name_is_prefixed_and_bounded(self):
        ...
        with autotest.step("Assert: the name is prefixed, bounded and alphanumeric"):
            assert_true(name.startswith("student-"), "startswith")
            assert_equal(len(name), len("student-") + 16, "name count")
            assert_true(name[len("student-") :].isalnum(), "isalnum")
    ...
    def test_5bd75aa5_uuid_users_do_not_collide(self):
        ...
        with autotest.step("Assert: all 500 names are distinct"):
            assert_equal(len(names), 500, "names count")
    ...
    def test_dba25fbf_two_sessions_of_one_student_differ(self):
        ...
        with autotest.step("Assert: the two sessions never share one GNS3 account"):
            assert_not_equal(first_name, second_name, "sessions must not share a GNS3 user")
