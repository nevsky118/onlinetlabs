...
import { getRegisterSchema, type RegisterFormValues } from "../lib/schemas"
...
export function RegisterForm({
  ...
  const t = useTranslations("dashboard.auth.registerForm")
  ...
  const registerSchema = useMemo(() => getRegisterSchema(schemaT), [schemaT])
  ...
  const onSubmit = handleSubmit(async (data) => {
    ...
    await authClient.signUp.credential(
      { email: data.email, password: data.password, name: data.name },
      {
        ...
        onError: (ctx) =>
          setServerError(ctx.error.message ?? t("registerFailed")),
      }
    )
  })
