...
function SidebarTrigger({
  className,
  onClick,
  labelToggleSidebar = "Toggle Sidebar",
  ...props
}: React.ComponentProps<typeof Button> & { labelToggleSidebar?: string }) {
  ...
  return (
    ...
    >
      <PanelLeftIcon />
      <span className="sr-only">{labelToggleSidebar}</span>
    </Button>
  )
}
...
function SidebarRail({
  ...
  labelToggleSidebar = "Toggle Sidebar",
  ...props
}: React.ComponentProps<"button"> & { labelToggleSidebar?: string }) {
  const { toggleSidebar } = useSidebar()
  ...
  return (
    <button
      ...
      aria-label={labelToggleSidebar}
      tabIndex={-1}
      onClick={toggleSidebar}
      title={labelToggleSidebar}
      className={cn(
